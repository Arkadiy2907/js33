...
// Краткое описание: OPTIONS - это один из методов HTTP, который используется для получения информации о возможностях сервера или параметрах конкретного ресурса
// Когда вызывается: OPTIONS вызывается клиентом, чтобы узнать, какие методы запроса поддерживаются сервером для указанного ресурса и параметров, связанных с ресурсом. Не предназначен для получения содержимого ресурса.
// Передает: пустое тело запроса
// Принимает: не принимает никаких параметров
// Для получения: поддерживаемых методов запроса (Allow), разрешенных заголовков (Access-Control-Allow-Headers), разрешенных источников (Access-Control-Allow-Origin) и другие метаданные (информацию о версии сервера, поддерживаемых протоколах, поддержке сжатия)
...
// Т к в JavaScript существует "временная мертвая зона" и в ней переменные уже существуют, но не могут быть использованы до момента их объявления.
